...
Usage:
    python3 tools/ytwebscraper.py basketball --max-videos 15
    python3 tools/ytwebscraper.py "vintage camera" --max-videos 10 --frame-fps 0.5
"""
...
class _BKTree:
    """Minimal BK-tree for fast Hamming-distance nearest-neighbor lookup.
    Reduces perceptual dedup from O(n^2) to ~O(n log n)."""
    ...
    def add(self, val) -> None:
        ...
        while True:
            ...
            if d in node.children:
                ...
                node.children[d] = _BKNode(val)
                return

    def has_within(self, val, threshold: int) -> bool:
        if self.root is None:
            return False
        stack = [self.root]
        while stack:
...
def dedupe_perceptual(out_dir: Path, threshold: int = 5) -> int:
    """Perceptual-hash dedup using a BK-tree for fast Hamming-distance
    lookups. Two frames with phash distance <= `threshold` are considered
    duplicates; we keep the first."""
    try:
        import imagehash
    ...
    for path in files:
        try:
            ...
            continue
        if tree.has_within(h, threshold):
            path.unlink()
            removed += 1
        else:
            tree.add(h)
...
def run_yt_scraper(
    ...
    """Programmatic entry point. Returns count of frames in out_dir/review/keep/."""
    from typing import Callable  # noqa: F811

    def _emit(step: str, **kwargs: object) -> None:
        if on_progress:
    ...
    # Bootstrap
    _emit("bootstrap", message="Analyzing topic...")
    spec: TopicSpec | None = None
    try:
        spec = load_or_bootstrap_spec(out_dir, query)
    except Exception as exc:
        _emit("bootstrap", message=f"Bootstrap failed ({exc}); using defaults.")

    if spec is not None and spec.type != "object":
        _emit("error", message=f"Topic '{query}' classified as {spec.type!r} — not an object.")
        return 0

    queries = spec.queries if spec else [query]
    vlm_object = spec.vlm_object if spec else f"a {query}"
    spec_yolo_class = spec.yolo_class if spec else None

    # Search & download
    if youtube_url:
        # Direct URL mode — skip search/score, just download and extract
        _emit("downloading", message=f"Downloading video...")
        video_path = download_video(youtube_url.split("v=")[-1].split("&")[0], video_dir)
        if video_path:
            _emit("extracting", message="Extracting frames...")
            extract_frames(video_path, out_dir, frame_fps=frame_fps)
            video_path.unlink(missing_ok=True)
        else:
            _emit("error", message="Video download failed.")
            return 0
    else:
        _emit("searching", message=f"Searching YouTube for {len(queries)} queries...")
        all_candidates = search_all_queries(queries, per_query=20, workers=6)
        filtered = [
            v for v in all_candidates.values()
            if pass_metadata_filter(v, 60, 1800)
        ]
        _emit("searching", message=f"{len(filtered)}/{len(all_candidates)} pass metadata filter.")

        if not filtered:
            _emit("done", message="No videos passed filter.", images_acquired=0)
            return 0

        _emit("scoring", message=f"Scoring {len(filtered)} candidates with Gemini...")
        scored = score_candidates_with_gemini(filtered, query, vlm_object)
        scored.sort(key=lambda x: -x[1])
        top = scored[:max_videos]

        _emit("downloading", message=f"Processing {len(top)} videos...")
        total_frames = 0
        with ThreadPoolExecutor(max_workers=2) as dl_pool:
            future_to_info = {
                dl_pool.submit(download_video, v["id"], video_dir): (i, v, score)
                for i, (v, score) in enumerate(top, 1)
            }
            for future in as_completed(future_to_info):
                i, v, score = future_to_info[future]
                video_path = future.result()
                if video_path is None:
                    continue
                _emit("extracting", message=f"Extracting frames from video {i}/{len(top)}...",
                      current=i, total=len(top))
                nframes = extract_frames(video_path, out_dir, frame_fps=frame_fps)
                total_frames += nframes
                video_path.unlink(missing_ok=True)

    # Dedup
    raw = sum(1 for p in out_dir.iterdir() if p.is_file() and not p.name.startswith("."))
    _emit("dedup", message=f"Deduplicating {raw} frames...")
    dedupe_by_hash(out_dir)
    dedupe_perceptual(out_dir, threshold=5)

    after_dedup = sum(1 for p in out_dir.iterdir() if p.is_file() and not p.name.startswith("."))
    if after_dedup == 0:
        _emit("done", message="No frames remaining after dedup.", images_acquired=0)
        return 0

    # YOLO
    yolo_class = spec_yolo_class
    if yolo_class is None:
        obj_q = vlm_object
        _emit("filtering", message=f"OWL-ViT detection ({obj_q})...")
        try:
        ...
        _emit("filtering", message=f"YOLO detection ({yolo_class})...")
        try:
            require_yolo_detection(
                out_dir, coco_class=yolo_class,
    ...
    final_count = sum(1 for p in keep_dir.iterdir() if p.is_file()) if keep_dir.is_dir() else 0
    _emit("done", message=f"Acquired {final_count} frames.", images_acquired=final_count)
    return final_count
...
def main() -> int:
    ...
    parser.add_argument("--yolo-model", default=str(_PROJECT_ROOT / "models" / "yolov8m.pt"))
    ...
    if not args.skip_download:
        ...
        # Download + extract frames — 2 concurrent downloads so the next
        # video downloads while the current one is being extracted.
        print(f"\nProcessing {len(top)} videos (2 concurrent downloads)...")
        total_frames = 0
        with ThreadPoolExecutor(max_workers=2) as dl_pool:
            future_to_info = {
                dl_pool.submit(
                    download_video, v["id"], video_dir, max_height=args.max_height,
                ): (i, v, score)
                for i, (v, score) in enumerate(top, 1)
            }
            for future in as_completed(future_to_info):
                ...
                video_path = future.result()
                if video_path is None:
                    print(f"\n[{i}/{len(top)}] {vid} download failed — skipping")
                    continue
                size_mb = video_path.stat().st_size / 1e6
                print(f"\n[{i}/{len(top)}] {vid} (score={score:.1f}, dur={dur}s, "
                      f"{size_mb:.1f}MB) {title}")
