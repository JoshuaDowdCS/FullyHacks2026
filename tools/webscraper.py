...
Usage:
    ...
    python3 tools/webscraper.py basketball --engines bing --per-query 100 \\
        --require-yolo --vlm-review

    # All sources (Bing + Baidu + Open Images Dataset):
    python3 tools/webscraper.py basketball --engines bing baidu openimages \\
        --per-query 100 --openimages-max 5000 --require-yolo --vlm-review

    # Skip the LLM bootstrap (use hardcoded PRESETS):
    python3 tools/webscraper.py basketball --no-bootstrap --require-yolo --vlm-review

    # Force a fresh spec (topic drifted, or you edited the prompt):
    python3 tools/webscraper.py basketball --refresh-spec ...
"""
...
def _hash_file(path: Path) -> tuple[Path, str | None]:
    """Read and MD5-hash a single file. Returns (path, hex_digest) or (path, None) on error."""
    try:
...
def dedupe_by_hash(out_dir: Path, workers: int = 8) -> int:
    ...
    File hashing is parallelized across *workers* threads.
    Returns number removed."""
    seen: dict[str, Path] = {}
    removed = 0

    # Phase 1: seed from review buckets (parallel hash, no deletions)
    review_root = out_dir / "review"
    if review_root.is_dir():
        seed_files = [
            p for seed_dir in review_root.iterdir() if seed_dir.is_dir()
            for p in seed_dir.iterdir()
            if p.is_file() and not p.name.startswith(".")
        ]
        if seed_files:
            with ThreadPoolExecutor(max_workers=workers) as ex:
                for path, h in ex.map(_hash_file, seed_files):
                    if h is not None:
                        seen.setdefault(h, path)

    # Phase 2: hash new files in parallel, then dedupe sequentially
    new_files = sorted(
    ...
    if new_files:
        ...
        hashes: list[tuple[Path, str | None]] = []
        with ThreadPoolExecutor(max_workers=workers) as ex:
            hashes = list(ex.map(_hash_file, new_files))
        # Dedupe sequentially (preserves sorted-first-wins order)
...
def require_yolo_detection(
    ...
    Processes images in batches of *batch_size* for throughput.
    ...
    print(
        ...
        f"(conf>={confident_threshold}, maybe>={uncertain_threshold}, batch={batch_size})..."
    )

    for start in range(0, total, batch_size):
        chunk = files[start:start + batch_size]
        # Load images, track which ones are valid
        valid: list[tuple[Path, Image.Image, int, int]] = []
        for path in chunk:
            try:
                img = Image.open(path).convert("RGB")
                valid.append((path, img, img.size[0], img.size[1]))
            except Exception:
                path.unlink(missing_ok=True)
        ...
        # Batch predict
        try:
            results = model.predict([img for _, img, _, _ in valid], verbose=False)
        except Exception as exc:
            print(f"  batch predict error: {exc}; keeping {len(valid)} in place")
            kept += len(valid)
            processed += len(chunk)
            continue

        for (path, _, W, H), res in zip(valid, results):
...
def vlm_review(
    ...
    def _classify_one(path: Path) -> str:
        ...
        # Downscale large images to reduce upload time; yes/no classification
...
def run_scraper(
    ...
    vlm_review_enabled: bool = True,
    on_progress: "Callable[[str, dict], None] | None" = None,
) -> int:
    """Programmatic entry point. Returns count of images in out_dir/review/keep/."""
    ...
    _emit("dedup", message=f"Deduplicating {raw} images...")
    dupes = dedupe_by_hash(out_dir)
    after_dedupe = raw - dupes
    _emit("dedup", message=f"Removed {dupes} duplicates ({after_dedupe} unique).")

    if after_dedupe == 0:
        _emit("done", message="No images found.", images_acquired=0)
        return 0

    # CLIP filtering
    spec_scene = spec.scene_distractors if spec else None
    spec_photo = spec.photo_prompts if spec else None
    spec_vlm_obj = spec.vlm_object if spec else None
    spec_yolo = spec.yolo_class if spec else None
    ...
    if require_yolo:
        if spec_yolo is None:
            obj_q = spec_vlm_obj or f"a {query}"
            ...
            _emit("filtering", message=f"YOLO detection ({spec_yolo})...")
            require_yolo_detection(
                out_dir, coco_class=spec_yolo,
    ...
    # VLM review
    if vlm_review_enabled:
        vlm_topic = spec_vlm_obj or query
        if require_yolo:
    ...
    _emit("done", message=f"Acquired {final_count} images.", images_acquired=final_count)
    return final_count
...
def main() -> int:
    ...
    parser.add_argument("--yolo-model", default=str(_PROJECT_ROOT / "models" / "yolov8m.pt"),
                        help="YOLO weights file (default models/yolov8m.pt; ~50MB; "
